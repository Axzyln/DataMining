<x-app-layout>
    <x-slot name="header">Edit Recipe</x-slot>

    <div class="max-w-3xl" x-data="recipeForm({{ json_encode($recipe->ingredients->map(fn($i) => ['ingredient_name' => $i->ingredient_name, 'quantity' => (string) $i->quantity, 'unit' => $i->unit, 'is_critical' => $i->is_critical])->values()) }})">
        <form method="POST" action="{{ route('baker.recipes.update', $recipe) }}" class="space-y-6">
            @csrf @method('PUT')
            ...
            <div class="rounded-xl border border-stone-200 bg-white p-6 space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                              class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">{{ old('description', $recipe->description) }}</textarea>
                </div>

                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="block text-sm font-medium text-stone-700 mb-1">Yield Quantity <span class="text-red-500">*</span></label>
                        <input type="number" name="yield_quantity" value="{{ old('yield_quantity', $recipe->yield_quantity) }}" min="1" required
                               class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-stone-700 mb-1">Yield Unit <span class="text-red-500">*</span></label>
                        <input type="text" name="yield_unit" value="{{ old('yield_unit', $recipe->yield_unit) }}" required
                               class="w-full rounded-lg border border-stone-300 px-3 py-2 text-sm focus:border-amber-500 focus:outline-none focus:ring-1 focus:ring-amber-500">
                    </div>
                </div>
            </div>
    ...
    <script>
        function recipeForm(initialRows) {
            return {
                rows: initialRows.length ? initialRows : [{ ingredient_name: '', quantity: '', unit: '', is_critical: true }],
                addRow() {
